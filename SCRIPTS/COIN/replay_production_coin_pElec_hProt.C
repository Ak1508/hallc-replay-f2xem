void replay_production_coin_pElec_hProt (Int_t RunNumber = 0, Int_t MaxEvent = 0,const char* ftype="heep_check") {
  ...
  //----------------------------------------
  ifstream hbcmFile;                                                                                
  TString hbcmParamFile = Form("PARAM/HMS/BCM/Hbcmcurrent_%d.param",  RunNumber);       
  hbcmFile.open(hbcmParamFile);       
  
  ifstream pbcmFile;                                                                                
  TString pbcmParamFile = Form("PARAM/HMS/BCM/Pbcmcurrent_%d.param",  RunNumber);   
  ...
  if(RunNumber < 4361)
    {
      gHcDetectorMap->Load("MAPS/COIN/coin_comm18.map");
    }
  
  else{
    gHcDetectorMap->Load("MAPS/COIN/coin.map");  //Fall 2018
  }
 ...
 //Add Module to explicitly plot all TDC hits from the trigger signals
  ...
  // Define DEF-file+
  TString DefTreeFile=Form("DEF-files/COIN/%s.def",ftype);
  analyzer->SetOdefFile(DefTreeFile);
  // Define cuts file
  DefTreeFile="DEF-files/COIN/CUTS/coin_production_cuts.def";
  ...
  TString TemplateFile=Form("TEMPLATES/COIN/coin_production.template");
